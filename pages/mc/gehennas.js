...
class Gehennas extends Component {
  render() {
    return (
      <MainContainer>
        <Head title="Home" />
        <BossList bosses={mc.bosses} selected="Gehennas" />
        <ContentContainer
          panels={[
            "Overview",
            "Abilities",
...
const Overview = () => (
  <div>
    <img
      ...
      alt="Gehennas"
    />
    Gehennas is one of the four flamewaker captains of Ragnaros, and the first
    boss of Molten Core.
  </div>
);
const Abilities = () => (
  <div>
    ...
    <p>
      <strong>Gehennas's Curse</strong>
...
const Strategy = () => (
  <div>
    <p>
      ...
      some room for the pullers and the main tank. It is important when Gehennas
    ...
    <p>
      ...
      Gehennas in the opposite direction of his guards. Generally the back cove
      is where the guards are pulled, while Gehennas is taken to another alcove
      on the east, far from the main raid. Each of the guards must then be
      picked up by two offtanks and dispatched accordingly before Gehennas is
      targeted.
    </p>
    <br />
    <p>
      Mages will not be contributing any large part of the DPS in this fight as
      they will be tied up with removing the curse that Gehennas casts on the
    ...
    <br />
    <p>
      Once the two guards are defeated, all DPS will shift over to Gehennas.
    ...
    <br />
    <p>
      Gehennas's debuffs have a range of 40 yards. It is possible for every
      healer (except the MT-decurser), and properly spec'ed hunters to stay out
      of Gehennas's curse-range, so with a good group you can minimize every
    ...
    <br />
    <p>
      If Gehennas is not defeated before the group wipes, the guards will
      respawn.{" "}
    </p>
  </div>
);
