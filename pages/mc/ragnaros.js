...
class Ragnaros extends Component {
  render() {
    return (
      <MainContainer>
        <Head title="Home" />
        <BossList bosses={mc.bosses} selected="Ragnaros" />
        <ContentContainer
          panels={[
            "Overview",
            "Abilities",
...
const Overview = () => (
  <div>
    <img
      ...
      alt="lucifron"
    />
    Ragnaros is one of the four flamewaker captains of Ragnaros, and the first
    boss of Molten Core.
  </div>
);
const Abilities = () => (
  <div>
    ...
    <p>
      <strong>Ragnaros's Curse</strong>
...
const Strategy = () => (
  <div>
    <p>
      ...
      some room for the pullers and the main tank. It is important when Ragnaros
      is pulled that he not come near the main raid groupimport Ragnaros from './ragnaros';
 or there could be an
    ...
    <p>
      ...
      Ragnaros in the opposite direction of his guards. Generally the back cove
      is where the guards are pulled, while Ragnaros is taken to another alcove
      on the east, far from the main raid. Each of the guards must then be
      picked up by two offtanks and dispatched accordingly before Ragnaros is
      targeted.
    </p>
    <br />
    <p>
      Mages will not be contributing any large part of the DPS in this fight as
      they will be tied up with removing the curse that Ragnaros casts on the
    ...
    <br />
    <p>
      Once the two guards are defeated, all DPS will shift over to Ragnaros.
    ...
    <br />
    <p>
      Ragnaros's debuffs have a range of 40 yards. It is possible for every
      healer (except the MT-decurser), and properly spec'ed hunters to stay out
      of Ragnaros's curse-range, so with a good group you can minimize every
    ...
    <br />
    <p>
      If Ragnaros is not defeated before the group wipes, the guards will
      respawn.{" "}
    </p>
  </div>
);
