...
class Garr extends Component {
  render() {
    return (
      <MainContainer>
        <Head title="Home" />
        <BossList bosses={mc.bosses} selected="Garr" />
        <ContentContainer
          panels={[
            "Overview",
            "Abilities",
...
const Overview = () => (
  <div>
    <img
      ...
      alt="Garr"
    />
    Garr is one of the four flamewaker captains of Ragnaros, and the first
    boss of Molten Core.
  </div>
);
const Abilities = () => (
...
const Strategy = () => (
  <div>
    <p>
      ...
      some room for the pullers and the main tank. It is important when Garr
    ...
    <p>
      ...
      Garr in the opposite direction of his guards. Generally the back cove
      is where the guards are pulled, while Garr is taken to another alcove
      on the east, far from the main raid. Each of the guards must then be
      picked up by two offtanks and dispatched accordingly before Garr is
      targeted.
    </p>
    <br />
    <p>
      Mages will not be contributing any large part of the DPS in this fight as
      they will be tied up with removing the curse that Garr casts on the
    ...
    <br />
    <p>
      Once the two guards are defeated, all DPS will shift over to Garr.
    ...
    <br />
    <p>
      Garr's debuffs have a range of 40 yards. It is possible for every
      healer (except the MT-decurser), and properly spec'ed hunters to stay out
      of Garr's curse-range, so with a good group you can minimize every
    ...
    <br />
    <p>
      If Garr is not defeated before the group wipes, the guards will
      respawn.{" "}
    </p>
  </div>
);
